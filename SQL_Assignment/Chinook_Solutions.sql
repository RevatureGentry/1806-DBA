...
--Write a SQL Query that determines the 5th highest grossing song in 2009, 2010, 2011, 2012, and 2013
	...
	SELECT SONG, PRICE
FROM
(SELECT TRACK.TRACKID, TRACK.NAME AS SONG, SUM(INVOICELINE.UNITPRICE) AS PRICE
    		FROM TRACK
    		INNER JOIN INVOICELINE
    		ON TRACK.TRACKID = INVOICELINE.TRACKID
INNER JOIN INVOICE
   		ON INVOICELINE.INVOICEID = INVOICE.INVOICEID
WHERE INVOICE.INVOICEDATE BETWEEN '01-JAN-09' AND '31-DEC-13'
GROUP BY TRACK.TRACKID, TRACK.NAME
ORDER BY PRICE DESC)
WHERE ROWNUM <= 5;
...
SELECT INVOICE.BILLINGCOUNTRY AS COUNTRY, GENRE.NAME AS GENRE, COUNT(INVOICELINE.TRACKID) AS DOWNLOADS
FROM INVOICE
INNER JOIN INVOICELINE
...
ORDER BY DOWNLOADS DESC, COUNTRY, GENRE;

--Write a SQL Query that determines the genres each salesperson contributes to most

SELECT EMPLOYEE, GENRE
FROM
(SELECT EMPLOYEE.EMPLOYEEID ID, (EMPLOYEE.FIRSTNAME || ' ' || EMPLOYEE.LASTNAME) AS EMPLOYEE, GENRE.NAME AS GENRE, SUM(INVOICELINE.UNITPRICE) AS SALES
    FROM EMPLOYEE
    INNER JOIN CUSTOMER
    ON EMPLOYEE.EMPLOYEEID = CUSTOMER.SUPPORTREPID
    INNER JOIN INVOICE
    ...
    INNER JOIN GENRE
    ON TRACK.GENREID = GENRE.GENREID
GROUP BY EMPLOYEE.EMPLOYEEID, (EMPLOYEE.FIRSTNAME || ' ' || EMPLOYEE.LASTNAME), GENRE.NAME
    ORDER BY SALES DESC)
WHERE ROWNUM <=3;
